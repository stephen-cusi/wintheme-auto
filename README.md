...
- **系统通知**：切换主题时弹出 Windows 原生 Toast 通知，支持自定义图标（通过 `win32_notif` 库 + Explorer AUMID）。可在主界面勾选开关。
...
notifications = true    # 切换主题时弹出 Windows 原生 Toast 通知
```
...
- 系统通知通过 `win32_notif` 库调用 WinRT Toast API，借用 Explorer AUMID 发送，自带自定义图标覆盖（`appLogoOverride`）。
